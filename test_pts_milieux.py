...
class Grid:
    ...
    def K_lenia(self, mu_filter: float, sigma_filter: float, a = 'fft'):
        ...
        All the informations on the growth function applied to a chanel are listed in the following order: [ filter, c0, h, m, s ]  
    ...
    def compute_next_iteration(self, K_lenia, a = 'fft', dt = 0.1):
            ...
            if self.channels == 1:
                ...
                if type(a) == str: # If we use a growth function in all channels
                    ...
                    for channel, channel_kernels in enumerate(K_lenia):
                     ...
                     #   for channel, channel_kernels in enumerate(K_lenia):
                      #      for kernel in channel_kernels:
                       #         E = np.fft.ifft2( kernel[iKERNEL] * np.fft.fft2(self.pixels_pts_milieux[:, :, kernel[iC0]] ))
                        #        E = np.real(E)
                         #       G[:,:,channel] += ( -1 + 2*Grid.gauss(E, kernel[iMU], kernel[iSIGMA]) ) * kernel[iH] 
        ...
        """
        if a != 'pacman' and a!='conv' and a!='fft' and a!='target': # Try "point milieux" scheme with orbium for instance(uncomment lines 162-171)
            ...
            if self.nbr_iter%2 == 0:
                self.pixels_pts_milieux = np.clip(Vitalite + dt * G / 2, 0, 1)
            else:
                self.pixels = np.clip(Vitalite + dt * G, 0, 1)
        self.nbr_iter += 1
