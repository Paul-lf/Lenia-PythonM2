...
class Grille:
    ...
    def compute_next_iteration(self):
        ...
        # solution np.roll pour voisins
        grille_1 = self.cells
        
        grille_2 = np.roll(self.cells, 1, axis=0)
        grille_3 = np.roll(self.cells, -1, axis=0)
        
        grille_4 = np.roll(self.cells, 1, axis=1)
        grille_5 = np.roll(self.cells, -1, axis=1)
        
        grille_6 = np.roll(np.roll(self.cells, 1, axis=0), 1, axis=1)
        grille_7 = np.roll(np.roll(self.cells, -1, axis=0), -1 ,axis=1)
       
        grille_8 = np.roll(np.roll(self.cells, -1, axis=0), 1 ,axis=1)
        grille_9 = np.roll(np.roll(self.cells, 1, axis=0), -1 ,axis=1)
        
        voisins = grille_2 +  grille_3 +  grille_4 +  grille_5 +  grille_6 +  grille_7 +  grille_8 +  grille_9 
        ...
        #for i in range(ny):
         ...
         #   for j in range(nx):
          ...
          #      if self.cells[i,j] == 1: # Si la cellule est vivante
           #         if (voisins[i, j] < 2) or (voisins[i,j]>3):
             ...
             #           diff_cells.append(i*nx+j)
              #      else:
               #         next_cells[i,j] = 1 # Sinon elle reste vivante
                #elif voisins[i,j] == 3: # Cas où cellule morte mais entourée exactement de trois vivantes
